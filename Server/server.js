            ...
            if(req.url == "/upload"){
                ...
                form.on("file", function (field, files) {
                    ...
                    if(extension == "png" || extension == "jpg"){
                        filesNamesList.push(`${files.name} -> okladka.jpg`)
                    } else {
                        filesNamesList.push(files.name)
                    }
                })
                form.parse(req, function (err, fields, files) {
                    ...
                    if(files.file instanceof Array){
                        files.file.forEach( e => {
                            ...
                            if(extension == "png" || extension == "jpg"){
                                fs.renameSync(form.uploadDir + e.path.substr(e.path.lastIndexOf("upload")), `${dir}/okladka.jpg`)
                            } else {
                                fs.renameSync(form.uploadDir + e.path.substr(e.path.lastIndexOf("upload")), `${dir}/${e.name}`)
                            }
                        })
                        ...
                        if(extension == "png" || extension == "jpg"){
                            fs.rename(form.uploadDir + files.file.path.substr(files.file.path.lastIndexOf("upload")), `${dir}/okladka.jpg`, (err) => {
                                res.writeHead(200, { "content-type": "application/json;charset=utf-8" });
                                res.end(JSON.stringify(filesNamesList))
                            })
                        } else {
                            fs.rename(form.uploadDir + files.file.path.substr(files.file.path.lastIndexOf("upload")), `${dir}/${files.file.name}`, (err) => {
                                res.writeHead(200, { "content-type": "application/json;charset=utf-8" });
                                res.end(JSON.stringify(filesNamesList))
                            })
                        }
